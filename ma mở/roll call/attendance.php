...
<div class="checkin-wrap">
    ...
    <div class="checkin-grid">
        ...
        <div class="right-panel">
            ...
            <!-- QR SCANNER MODAL -->
            <div id="qr-scan-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:9999;align-items:center;justify-content:center;flex-direction:column;">
              <div style="background:#1e1b4b;border:1px solid rgba(99,102,241,0.4);border-radius:20px;padding:28px 24px;max-width:400px;width:92%;text-align:center;position:relative;">
                <button onclick="closeQrScanner()" style="position:absolute;top:12px;right:16px;background:none;border:none;font-size:1.4rem;cursor:pointer;color:rgba(255,255,255,0.5);">✕</button>
                <h2 style="color:#fff;font-size:1.1rem;font-weight:800;margin-bottom:4px;">📷 Quét Mã QR Điểm Danh</h2>
                <p style="color:rgba(255,255,255,0.45);font-size:0.78rem;margin-bottom:16px;">Hướng camera vào mã QR của giáo viên</p>
                <div style="position:relative;border-radius:14px;overflow:hidden;background:#000;margin-bottom:14px;">
                  <video id="qr-video" autoplay playsinline muted style="width:100%;max-height:280px;object-fit:cover;display:block;"></video>
                  <!-- Corner frames -->
                  <div style="position:absolute;inset:0;pointer-events:none;">
                    <div style="position:absolute;top:12px;left:12px;width:40px;height:40px;border-top:3px solid #818cf8;border-left:3px solid #818cf8;border-radius:4px 0 0 0;"></div>
                    <div style="position:absolute;top:12px;right:12px;width:40px;height:40px;border-top:3px solid #818cf8;border-right:3px solid #818cf8;border-radius:0 4px 0 0;"></div>
                    <div style="position:absolute;bottom:12px;left:12px;width:40px;height:40px;border-bottom:3px solid #818cf8;border-left:3px solid #818cf8;border-radius:0 0 0 4px;"></div>
                    <div style="position:absolute;bottom:12px;right:12px;width:40px;height:40px;border-bottom:3px solid #818cf8;border-right:3px solid #818cf8;border-radius:0 0 4px 0;"></div>
                    <div id="qr-scan-line" style="position:absolute;left:12px;right:12px;height:2px;background:linear-gradient(90deg,transparent,#818cf8,transparent);animation:scanLine 2s linear infinite;top:50%;"></div>
                  </div>
                </div>
                <div id="qr-scan-status" style="color:rgba(255,255,255,0.5);font-size:0.82rem;margin-bottom:14px;">⏳ Đang khởi động camera...</div>
                <div id="qr-scan-result" style="display:none;padding:14px;border-radius:12px;margin-bottom:14px;font-weight:700;font-size:0.95rem;"></div>
                <p style="color:rgba(255,255,255,0.3);font-size:0.72rem;">Thư viện: jsQR — quét tự động, không cần nhấn nút</p>
              </div>
            </div>
            <style>
            ...
            <script>
            var _qrScanStream = null;
            var _qrScanRAF = null;
            var _qrScanDone = false;
            ...
            async function startQrScanCamera() {
                try {
                    ...
                    document.getElementById('qr-scan-status').textContent = '❌ Không truy cập được camera: ' + e.message;
                }
            }
            ...
            function scanQrFrame() {
                if (_qrScanDone) return;
                var vid = document.getElementById('qr-video');
                if (!vid || !vid.videoWidth) { _qrScanRAF = requestAnimationFrame(scanQrFrame); return; }
                ...
                if (code && code.data && code.data.includes('action=qr_checkin')) {
                    _qrScanDone = true;
                    stopQrScanCamera();
            ...
            function submitQrToken(token) {
                ...
                fetch('?action=qr_checkin&token=' + encodeURIComponent(token), { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        setTimeout(function(){ closeQrScanner(); location.reload(); }, 2200);
                    } else {
                        el.style.background = 'rgba(248,113,113,0.15)';
                        el.style.border = '1px solid rgba(248,113,113,0.4)';
                ...
                .catch(() => {
                    // Fallback: redirect to QR checkin page
