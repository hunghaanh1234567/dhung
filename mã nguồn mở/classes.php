...
function pageTeacherStudents($db) {
    ...
    // Lấy danh sách học viên kèm thống kê điểm danh
    $students = $db->query("
        SELECT
            ...
            (SELECT COUNT(*) FROM attendance_records ar
             JOIN attendance_sessions s2 ON ar.session_id=s2.id
             WHERE ar.user_id=u.id AND s2.course_id=c.id) as total_sessions,
            (SELECT COUNT(*) FROM attendance_records ar
             JOIN attendance_sessions s2 ON ar.session_id=s2.id
             WHERE ar.user_id=u.id AND s2.course_id=c.id AND ar.status='absent') as absent_count,
            (SELECT COUNT(*) FROM attendance_records ar
             JOIN attendance_sessions s2 ON ar.session_id=s2.id
             WHERE ar.user_id=u.id AND s2.course_id=c.id AND (ar.status='present' OR ar.status='late')) as present_count,
            (SELECT COUNT(*) FROM attendance_records ar
             JOIN attendance_sessions s2 ON ar.session_id=s2.id
             WHERE ar.user_id=u.id AND s2.course_id=c.id AND ar.status='excused') as excused_count
        FROM enrollments e
        JOIN users u ON e.user_id=u.id
        JOIN courses c ON e.course_id=c.id
        WHERE $where
        ORDER BY absent_count DESC, e.enrolled_at DESC
    ")->fetchAll(PDO::FETCH_ASSOC);

    // Đếm học viên vắng nhiều (>= 3 buổi) để cảnh báo
    $warnCount = count(array_filter($students, fn($s) => (int)$s['absent_count'] >= 3));
?>
<div class="container" style="padding-top:32px;padding-bottom:48px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            <?php if ($warnCount > 0): ?>
            <span class="badge badge-danger" style="font-size:0.85rem;padding:7px 14px;">⚠️ <?= $warnCount ?> học viên vắng nhiều</span>
            <?php endif; ?>
    ...
    <div class="alert alert-error" style="margin-bottom:20px;">
        ⚠️ Có <strong><?= $warnCount ?></strong> học viên vắng từ 3 buổi trở lên. Hãy kiểm tra và liên hệ kịp thời!
    </div>
    <?php endif; ?>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="text-align:center;">❌ Buổi Nghỉ</th>
        ...
        <?php foreach($students as $s):
            ...
            $excused = (int)$s['excused_count'];
            $attendRate = $total > 0 ? round(($present + $excused) / $total * 100) : null;
            $rateColor = $attendRate === null ? 'var(--gray)' : ($attendRate >= 80 ? 'var(--success)' : ($attendRate >= 60 ? '#f59e0b' : 'var(--danger)'));
            $rowStyle = $absent >= 3 ? 'background:#fff5f5;' : '';

            // Điểm trung bình lần đầu của học viên trong khóa học này
            $avgStmt = $db->prepare("
            ...
            ");
            $avgStmt->execute([$s['id'], $s['course_id']]);
            $avgRow = $avgStmt->fetch(PDO::FETCH_ASSOC);
            $avgOn10 = ($avgRow && $avgRow['avg_score'] !== null)
                ? number_format($avgRow['avg_score'] / 10, 1) : null;
            $avgColor = $avgOn10 === null ? 'var(--gray)' : ($avgOn10 >= 8 ? 'var(--success)' : ($avgOn10 >= 5 ? '#f59e0b' : 'var(--danger)'));
        ?>
        <tr style="<?= $rowStyle ?>">
            <td>
                <strong><?= h($s['full_name']) ?></strong>
                <?php if ($absent >= 3): ?>
                <span title="Vắng nhiều" style="margin-left:4px;">⚠️</span>
                <?php endif; ?>
                <br><span style="font-size:0.75rem;color:var(--gray);"><?= h($s['email']) ?></span>
            </td>
            <td style="font-size:0.875rem;"><?= h($s['course_title']) ?></td>
            <td style="text-align:center;min-width:110px;">
                <div style="display:flex;align-items:center;gap:6px;justify-content:center;">
                    <div class="progress" style="flex:1;max-width:60px;height:6px;"><div class="progress-bar" style="width:<?= $progress ?>%"></div></div>
                    <span style="font-size:0.78rem;font-weight:700;"><?= $progress ?>%</span>
                </div>
            </td>
            <td style="text-align:center;">
                <?php if ($avgOn10 !== null): ?>
                <span style="font-size:1rem;font-weight:800;color:<?= $avgColor ?>;"><?= $avgOn10 ?>/10</span>
                <?php if ((int)$avgRow['quiz_count'] > 0): ?>
                <div style="font-size:0.7rem;color:var(--gray);"><?= $avgRow['quiz_count'] ?> bài</div>
                <?php endif; ?>
                <?php else: ?>
            ...
            <td style="text-align:center;">
                <?php if ($total > 0): ?>
                <span class="badge <?= $absent >= 3 ? 'badge-danger' : ($absent > 0 ? 'badge-warning' : 'badge-success') ?>"
                      style="font-size:0.82rem;padding:4px 12px;font-weight:800;">
                    <?= $absent ?>
                </span>
                <?php else: ?>
                <span style="color:var(--gray);font-size:0.8rem;">—</span>
            ...
            <td style="text-align:center;">
                ...
                <?php else: ?>
                <span style="color:var(--gray);font-size:0.8rem;">0</span>
                <?php endif; ?>
            </td>
            <td style="text-align:center;min-width:120px;">
                <?php if ($attendRate !== null): ?>
                <div style="display:flex;align-items:center;gap:6px;justify-content:center;">
                    <div style="flex:1;max-width:60px;">
                        <div class="progress" style="height:6px;">
                            <div class="progress-bar" style="width:<?= $attendRate ?>%;background:<?= $rateColor ?>;"></div>
                        </div>
                    </div>
                    <span style="font-size:0.8rem;font-weight:800;color:<?= $rateColor ?>;"><?= $attendRate ?>%</span>
                </div>
                <?php else: ?>
                <span style="color:var(--gray);font-size:0.8rem;">Chưa có</span>
                <?php endif; ?>
            </td>
            <td style="font-size:0.8rem;color:var(--gray);white-space:nowrap;"><?= date('d/m/Y', strtotime($s['enrolled_at'])) ?></td>
            <td style="text-align:center;">
                <form method="post" action="?action=unenroll_student"
                      onsubmit="return confirm('Xóa <?= h(addslashes($s['full_name'])) ?> khỏi \"<?= h(addslashes($s['course_title'])) ?>\"?\nThao tác này không thể hoàn tác!');"
                      style="margin:0;">
                    <input type="hidden" name="user_id"   value="<?= $s['id'] ?>">
                    <input type="hidden" name="course_id" value="<?= $s['course_id'] ?>">
                    <button type="submit" class="btn btn-sm"
                            style="background:#fee2e2;color:#dc2626;border:1px solid #fca5a5;padding:4px 10px;font-size:0.78rem;font-weight:700;border-radius:6px;cursor:pointer;white-space:nowrap;">
                        🗑 Xóa
                    </button>
                </form>
            </td>
        ...
        <tr><td colspan="10" style="text-align:center;color:var(--gray);padding:32px;">Chưa có học viên nào.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
    </div>
