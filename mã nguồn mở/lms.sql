...
-- ============================================================
-- PHẦN BỔ SUNG: Kiểm Tra Kết Thúc Học Kỳ
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `semester_exams` (
  ...
  `time_limit` INT DEFAULT 60 COMMENT 'Phút làm bài',
  `passing_score` INT DEFAULT 50 COMMENT 'Phần trăm điểm đậu',
  `start_time` DATETIME DEFAULT NULL,
  `end_time` DATETIME DEFAULT NULL,
  `shuffle_questions` TINYINT(1) DEFAULT 1,
  `shuffle_answers` TINYINT(1) DEFAULT 1,
  `show_result` TINYINT(1) DEFAULT 1 COMMENT 'Hiển thị đáp án sau khi nộp',
...
CREATE TABLE IF NOT EXISTS `exam_questions` (
  ...
  `explanation` TEXT DEFAULT NULL COMMENT 'Giải thích đáp án đúng',
  `order_num` INT DEFAULT 0,
  FOREIGN KEY(`exam_id`) REFERENCES `semester_exams`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Lượt thi của học viên (mỗi học viên chỉ thi 1 lần)
CREATE TABLE IF NOT EXISTS `exam_attempts` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `exam_id` INT NOT NULL,
  `user_id` INT NOT NULL,
  `score` DECIMAL(5,2) DEFAULT 0 COMMENT 'Phần trăm điểm đạt được',
  `correct_count` INT DEFAULT 0,
  `total_questions` INT DEFAULT 0,
  `passed` TINYINT(1) DEFAULT 0,
  `answers_json` LONGTEXT DEFAULT NULL COMMENT 'JSON: {question_id: dap_an_chon}',
  `duration_minutes` INT DEFAULT NULL COMMENT 'Thời gian thực tế làm bài',
  `submitted_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY `one_attempt_per_exam` (`exam_id`, `user_id`),
  FOREIGN KEY(`exam_id`) REFERENCES `semester_exams`(`id`) ON DELETE CASCADE,
  FOREIGN KEY(`user_id`) REFERENCES `users`(`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
