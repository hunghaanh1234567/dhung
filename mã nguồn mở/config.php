...
function initDB($db) {
    ...
    $db->exec("
        CREATE TABLE IF NOT EXISTS `quiz_attempts` (
            ...
            `is_first` TINYINT(1) DEFAULT 0 COMMENT '1 = lần thi đầu tiên của học viên',
            `answers` TEXT,
    ...
    ");
    // Tương thích DB cũ: thêm cột is_first nếu chưa có
    try {
        $db->exec("ALTER TABLE `quiz_attempts` ADD COLUMN `is_first` TINYINT(1) DEFAULT 0 COMMENT '1 = lan thi dau tien'");
    } catch(Exception $e) { /* cột đã tồn tại — bỏ qua */ }

    // Backfill: đánh dấu is_first=1 cho lần thi SỚM NHẤT của mỗi (user, quiz)
    // nếu chưa có bản ghi nào is_first=1 (dữ liệu cũ trước khi có cột này)
    $hasFirst = $db->query("SELECT COUNT(*) FROM quiz_attempts WHERE is_first = 1")->fetchColumn();
    if ((int)$hasFirst === 0) {
        // Lấy id nhỏ nhất (= sớm nhất) của mỗi cặp user+quiz rồi update
        $db->exec("
            ...
            ) first_ids ON qa.id = first_ids.min_id
            SET qa.is_first = 1
        ");
    }
    $db->exec("
